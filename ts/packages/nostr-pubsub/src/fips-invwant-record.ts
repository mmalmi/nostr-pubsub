...
export function encodeInvWantRecord(
  payload: Uint8Array,
...
export class InvWantRecordDecoder {
  ...
  push(bytes: Uint8Array, maxRecords: number): Uint8Array[] {
    ...
    if (!Number.isSafeInteger(attempted) || attempted > capacity) {
      ...
      throw validation(
        `record input is ${attempted} bytes, maximum buffered input is ${capacity}`,
      );
    }
    ...
    while (
      ...
      const declared = new DataView(
        this.buffer.buffer,
        this.buffer.byteOffset + consumed,
        INV_WANT_RECORD_PREFIX_BYTES,
      ).getUint32(0, false);
      if (declared > this.maxRecordBytes) {
        this.buffer = new Uint8Array();
        throw validation(
          `record declares ${declared} bytes, maximum is ${this.maxRecordBytes}`,
        );
      }
      const recordBytes = INV_WANT_RECORD_PREFIX_BYTES + declared;
      if (this.buffer.byteLength - consumed < recordBytes) break;
    ...
    if (consumed === this.buffer.byteLength) this.buffer = new Uint8Array();
    else if (consumed > 0) this.buffer = this.buffer.slice(consumed);
    return records;
  }
  ...
  get remainingCapacity(): number {
    return Math.max(
      0,
      this.maxRecordBytes + INV_WANT_RECORD_PREFIX_BYTES - this.buffer.byteLength,
    );
  }

  get hasCompleteRecord(): boolean {
    if (this.buffer.byteLength < INV_WANT_RECORD_PREFIX_BYTES) return false;
    const declared = new DataView(
      this.buffer.buffer,
