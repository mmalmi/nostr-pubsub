...
export type InvWantWireMessage =
  ...
  | { type: 'frame'; eventId: string; event: NostrEvent };
...
export class InvWantCodec {
  ...
  decode(payload: Uint8Array): InvWantWireMessage {
    ...
    if (!isRecord(value)) throw validation('invalid inv/want JSON: envelope must be an object');
  ...
  private checkWireLength(length: number): void {
    if (length > this.maxWireBytes) {
      throw validation(
        `inv/want wire payload is ${length} bytes, maximum is ${this.maxWireBytes}`,
      );
    }
  }
}

export function meshEventJsonBytes(event: NostrEvent): number {
  return new TextEncoder().encode(JSON.stringify(eventToWire(event))).byteLength;
}

function messageToWire(message: InvWantWireMessage): Record<string, unknown> {
  switch (message.type) {
    case 'inventory':
      return {
        type: 'inventory',
...
function messageFromWire(value: unknown): InvWantWireMessage {
  if (!isRecord(value)) throw validation('invalid inv/want JSON: message must be an object');
  switch (value.type) {
    case 'inventory':
      return {
        type: 'inventory',
...
function eventToWire(event: NostrEvent): Record<string, unknown> {
...
function eventFromWire(value: unknown): NostrEvent {
  if (!isRecord(value)) throw validation('invalid inv/want event structure');
...
function requireU8(value: unknown, field: string): number {
  ...
  if (number > 255) throw validation(`invalid inv/want ${field} ${number}`);
  return number;
}

function requireU16(value: unknown, field: string): number {
  const number = requireNonNegativeSafeInteger(value, field);
  if (number > 65_535) throw validation(`invalid inv/want ${field} ${number}`);
  return number;
}

function requireU32(value: unknown, field: string): number {
  const number = requireNonNegativeSafeInteger(value, field);
  if (number > 0xffff_ffff) throw validation(`invalid inv/want ${field} ${number}`);
  return number;
}
