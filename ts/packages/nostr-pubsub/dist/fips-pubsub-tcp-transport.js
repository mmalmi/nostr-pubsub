...
export class FipsPubsubTcpTransport {
    ...
    async connectPeer(peer, nowMs = Date.now()) {
        ...
        for (const [id, connection] of this.connections) {
            ...
            if (connection.peer === peer && state !== undefined && CONNECTING_STATES.has(state))
                return;
        }
    ...
    async abortPeer(peer) {
        ...
        const ids = [...this.connections]
            ...
            .map(([id]) => id);
        for (const id of ids) {
            await transport('abort TCP/FIPS Nostr pubsub peer', abortTcpConnectionIfPresent(this.tcp, id));
            this.connections.delete(id);
        }
        if (this.active.delete(peer))
            this.callbacks.disconnected(peer);
    ...
    async dispose() {
        ...
        for (const id of ids) {
            if (await this.tcp.state(id) !== undefined)
                await this.tcp.abort(id).catch(() => undefined);
        }
    ...
    scheduleDrive(poll) {
        if (this.disposed)
            return;
        void this.enqueueDrive(poll).catch((error) => this.callbacks.error(asError(error)));
    }
    enqueueDrive(poll) {
        const next = this.operation.then(async () => {
            if (this.disposed)
                return;
            if (poll)
                await transport('poll TCP/FIPS Nostr pubsub transport', this.tcp.poll());
            await this.driveReady(Date.now());
        });
        this.operation = next.catch(() => undefined);
        return next;
    }
    async driveReady(nowMs) {
    ...
    async refreshActive() {
        ...
        for (const peer of [...changed].sort()) {
            ...
            if (previous.has(peer))
                this.callbacks.disconnected(peer);
            if (next.has(peer))
                this.callbacks.connected(peer);
        }
    }
    async readActive(nowMs) {
        ...
        for (const [peer, id] of this.active) {
            ...
            for (let turn = 0; turn < MAX_READY_INPUT_TURNS && frameBudget > 0; turn += 1) {
                ...
                if (decoder.hasCompleteRecord || byteBudget === 0)
                    continue;
                const maximum = Math.min(decoder.remainingCapacity, STREAM_IO_CHUNK_BYTES, byteBudget);
                if (maximum === 0)
                    break;
                const bytes = await transport('read TCP/FIPS Nostr pubsub stream', this.tcp.read(id, maximum, nowMs));
            ...
            if (byteBudget === 0 || frameBudget === 0)
                break;
        }
    }
    async flushQueues(nowMs) {
        ...
        for (const [peer, id] of this.active) {
            while (budget > 0) {
                ...
                if (chunk === undefined)
                    break;
                const accepted = await transport('write TCP/FIPS Nostr pubsub stream', this.tcp.write(id, chunk, nowMs));
                if (accepted === 0)
                    break;
                budget -= accepted;
    ...
    ensureOpen() {
        if (this.disposed)
            throw storage('TCP/FIPS Nostr pubsub transport is disposed');
    }
}
...
//# sourceMappingURL=fips-pubsub-tcp-transport.js.map