...
pub(super) fn build_sim_topology(
    ...
    topology.hybrid.false_supernode_count = config.false_supernode_count.min(config.attacker_count);
    ...
    topology.hybrid.max_peer_degree = config.supernode_links_per_peer.max(4);
...
pub(super) fn connected_quiet_attackers<'a>(
    ...
    let false_candidates = topology
        .false_supernode_candidates
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let mut attackers = (0..config.attacker_count)
        ...
        .collect::<Vec<_>>();
    attackers.sort_by_key(|attacker| (!false_candidates.contains(attacker), *attacker));
    attackers.into_iter()
}
...
fn swap_topology_nodes(topology: &mut TopologyResult, left: usize, right: usize) {
    ...
    replace_node(&mut topology.false_supernode_candidates, left, right);
}
...
fn node_degree_cap(config: &SimulationConfig, topology: &TopologyResult, node: usize) -> usize {
    match (config.topology, topology.roles[node]) {
        ...
        (TopologyStrategy::HybridSupernodes, NodeRole::Peer) => {
            config.supernode_links_per_peer.max(4)
        }
